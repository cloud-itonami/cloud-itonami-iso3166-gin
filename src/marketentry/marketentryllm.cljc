(ns marketentry.marketentryllm
  ...
  the Republic of Guinea (GIN) public-sector market-entry compliance
  actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "nifp-verified?=" (:nifp-verified? e)
                        " prior-armp-pecuniary-sanction?=" (:prior-armp-pecuniary-sanction? e)
                        " armp-sanction-amount=" (:armp-sanction-amount e)
                        " armp-sanction-base-amount=" (:armp-sanction-base-amount e)
