...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (GIN, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up pecuniary-sanction-range-violated) ==")
    ...
    (println "== filing/submit eng-4 (declared ARMP sanction is 5% of base, outside Art.159's 1-2% range -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up NIFP-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (NIFP registration unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (sets up an IN-range pecuniary sanction -- clean) ==")
    ...
    (println "== filing/submit eng-6 (declared ARMP sanction is 1.5% of base, WITHIN Art.159's 1-2% range -- clean submit, still escalates for approval) ==")
