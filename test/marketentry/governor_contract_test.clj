  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Conakry Négoce SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Conakry Négoce SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest pecuniary-sanction-range-violated-is-held-and-unoverridable
  (testing "declared ARMP pecuniary sanction (5% of base amount) falls outside Code des Marchés Publics Art.159's 1%-2% range -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:pecuniary-sanction-range-violated} (-> (store/ledger db) last :basis)))
...
(deftest nifp-record-unverified-is-held-and-unoverridable
  (testing "unverified DGI NIFP registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:nifp-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest pecuniary-sanction-within-range-is-not-gated-by-the-flagship-check
  (testing "eng-6 declares a prior ARMP sanction that IS within Art.159's 1%-2% range (1.5%) -- the flagship check never fires for it"
    (let [[_db actor] (fresh)
